...
import { Content, formatErrorResponse, formatResponse, textContent, toolResponse } from "../utils";
...
import {  
  ...
  //ItemsUpdateItemsLiveRequest,
   ItemsDeleteItemsRequest, ItemsListItemsRequest,
     ItemsUpdateItemsRequest } from "webflow-api/api/resources/collections/resources/items";
...
export function registerCmsTools(
  ...
  const createCollection = async (arg:{siteId:string, request: CollectionsCreateRequest})=>{
  ...
  const createCollectionStaticField = async (arg:{collection_id:string, request: FieldCreate})=>{
  ...
  const updateCollectionField = async (arg:{collection_id:string, field_id:string, request: FieldUpdate})=>{
  ...
  const listCollectionItems = async (arg:{collection_id:string, request: ItemsListItemsRequest})=>{
  ...
  const createCollectionItems = async (arg:{collection_id:string, request: {
    cmsLocaleIds?: string[];
    isArchived?: boolean;
    isDraft?: boolean;
    fieldData: {
  ...
  const updateCollectionItems = async (arg:{collection_id:string, request: ItemsUpdateItemsRequest})=>{
  ...
  const publishCollectionItems = async (arg:{collection_id:string, request: {
    itemIds: string[];
  }})=>{
  ...
  server.tool("data_cms_tool","Data tool - CMS tool to perform actions like get collection list, get collection details, create collection, create collection static field, create collection option field, create collection reference field, update collection field, create collection item live, update collection items live, list collection items, create collection item, update collection items, publish collection items, delete collection item",{
    actions:z.array(
      z.object({
        ...
        get_collection_list:z.object({
          ...SiteIdSchema,
        }).optional().describe("List all CMS collections in a site. Returns collection metadata including IDs, names, and schemas."),
        // GET https://api.webflow.com/v2/collections/:collection_id
        get_collection_details:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
        }).optional().describe("Get detailed information about a specific CMS collection including its schema and field definitions."),
        ...
        }).optional().describe("Create a new CMS collection in a site with specified name and schema."),
        // POST https://api.webflow.com/v2/collections/:collection_id/fields
        create_collection_static_field:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: StaticFieldSchema,
        }).optional().describe("Create a new static field in a CMS collection (e.g., text, number, date, etc.)."),
        // POST https://api.webflow.com/v2/collections/:collection_id/fields
        create_collection_option_field:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: OptionFieldSchema,
        }).optional().describe("Create a new option field in a CMS collection with predefined choices."),
        // POST https://api.webflow.com/v2/collections/:collection_id/fields
        create_collection_reference_field:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: ReferenceFieldSchema,
        }).optional().describe("Create a new reference field in a CMS collection that links to items in another collection."),
        // PATCH https://api.webflow.com/v2/collections/:collection_id/fields/:field_id
        update_collection_field:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          field_id: z.string().describe("Unique identifier for the Field."),
          request: WebflowCollectionsFieldUpdateSchema,
        }).optional().describe("Update properties of an existing field in a CMS collection."),
        ...
        list_collection_items:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request:z.object({
      cmsLocaleId: z
        .string()
      ...
      sortOrder: WebflowCollectionsItemsListItemsRequestSortOrderSchema,
          }).optional().describe("Filter and sort items in a CMS collection."),
        }).optional().describe("List items in a CMS collection with optional filtering and sorting."),
        // POST https://api.webflow.com/v2/collections/:collection_id/items/bulk
        create_collection_items:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: z.object({
            cmsLocaleIds:z.array(z.string()).optional().describe("Unique identifier for the locale of the CMS Item."),
            ...
            fieldData:z.array(z.record(z.any()).and(z.object({
        ...
        }).optional().describe("Create new items in a CMS collection as drafts."),
        //PATCH https://api.webflow.com/v2/collections/:collection_id/items
        update_collection_items:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: WebflowCollectionsItemsUpdateItemsRequestSchema.describe("Array of items to be updated."),
        }).optional().describe("Update existing items in a CMS collection as drafts."),
        // POST https://api.webflow.com/v2/collections/:collection_id/items/publish
        publish_collection_items:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: z.object({
            itemIds: z.array(z.string()).describe("Array of item IDs to be published."),
          }).describe("Array of items to be published."),
        }).optional().describe("Publish existing items in a CMS collection as drafts."),
       // DEL https://api.webflow.com/v2/collections/:collection_id/items
        delete_collection_items:z.object({
          collection_id: z.string().describe("Unique identifier for the Collection."),
          request: z.object({
        ...
        }).optional().describe("Delete existing items in a CMS collection as drafts."),
      })
    )
  },async ({actions})=>{
    ...
    for(const action of actions){
      ...
      }else if(action.get_collection_details){
        const content = await getCollectionDetails(action.get_collection_details);
        result.push(textContent(content));
      }else if(action.create_collection){
        const content = await createCollection(action.create_collection);
        result.push(textContent(content));
      }else if(action.create_collection_static_field){
        const content = await createCollectionStaticField(action.create_collection_static_field);
        result.push(textContent(content));
      }else if(action.create_collection_option_field){
        const content = await createCollectionOptionField(action.create_collection_option_field);
        result.push(textContent(content));
      }else if(action.create_collection_reference_field){
        const content = await createCollectionReferenceField(action.create_collection_reference_field);
        result.push(textContent(content));
      }
      else if(action.update_collection_field){
        const content = await updateCollectionField(action.update_collection_field);
        result.push(textContent(content));
      }
      ...
      //   result.push(textContent(content));
      // }
      else if(action.list_collection_items){
        ...
        result.push(textContent(content));
      }else if(action.create_collection_items){
        ...
        result.push(textContent(content));
      }else if(action.update_collection_items){
        ...
        result.push(textContent(content));
      }else if(action.publish_collection_items){
        ...
        result.push(textContent(content));
      }else if(action.delete_collection_items){
