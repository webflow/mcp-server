...
import { z } from "zod";
import { requestOptions } from "../../../config/constant";
import {
  WebflowPageDomWriteNodesItemSchema,
  WebflowPageSchema,
} from "./schemas";
import { formatErrorResponse,  
  formatResponse } from "../../../utils/formatResponse";
...
export function registerPagesTools(
  ...
  server.tool(
    ...
    async ({ page_id, localeId }) => {
      try {
        const response =
          await getClient().pages.getMetadata(
            page_id,
            {
              localeId,
            },
  ...
  server.tool(
    ...
    async ({ page_id, localeId, body }) => {
      try {
        const response =
          await getClient().pages.updatePageSettings(
            page_id,
            {
              localeId,
              body,
  ...
  server.tool(
    ...
    async ({ page_id, localeId, nodes }) => {
      try {
        const response =
          await getClient().pages.updateStaticContent(
            page_id,
            {
              localeId,
              nodes,
